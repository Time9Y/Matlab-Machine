...
function model = regRF_train(p_train, t_train, ntree, mtry, extra_options)
  ...
  % if ~exist('norm_votes', 'var');   norm_votes = TRUE;  end
    ...
    if ~exist('mtry', 'var') || mtry < 0 || mtry > size(p_train, 2)
        mtry = max(floor(size(p_train, 2) / 3), 1);
        DEFAULTS_ON = 1;
    end
        
    [N, D] = size(p_train);
    
    if length(unique(t_train)) <= 5  
    ...
    if length(t_train) ~= N || isempty(t_train)
...
%        nsample = n;
	...
	[ldau, rdau,nodestatus, nrnodes, upper, avnode,...
        ...
        = mexRF_train (p_train', t_train, ntree, mtry, sampsize, nodesize,...
    ...
    model.rsq = 1 - mse / (var(t_train) * (N - 1) / N);
	clear mexRF_train